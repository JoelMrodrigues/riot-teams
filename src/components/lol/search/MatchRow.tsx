...
/** Ligne d'historique non-interactive : champion, file, KDA, CS, résultat. */
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs" style={{ color: 'var(--lol-text-muted)' }}>
          {QUEUE_LABELS[match.queue]} · {minutes} min
        </p>
      </div>
      <div className="text-right">
        <p className="text-sm font-bold" style={{ color: 'var(--lol-text)' }}>
          {match.kills}/{match.deaths}/{match.assists}
        </p>
        <p className="text-xs" style={{ color: 'var(--lol-text-muted)' }}>
          {match.kda} KDA · {match.csPerMin} cs/min
        </p>
      </div>
      <span
        className="w-8 text-center text-sm font-bold"
        style={{ color: edge, fontFamily: 'Rajdhani, sans-serif' }}
      >
        {match.win ? 'V' : 'D'}
      </span>
    </div>
