...
export function LolSoloProfileCard(): React.JSX.Element {
  return (
    ...
    >
      ...
      {/* Rank banner */}
      <div className="mb-5 flex items-center justify-between rounded-2xl px-4 py-3" style={{ background: A.gradient }}>
        <div>
      ...
      <div className="mb-5 flex flex-col gap-2">
        {SOLO_DEMO.topChampions.map((c) => (
          <div key={c.key} className="flex items-center gap-3 rounded-xl px-3 py-2" style={{ background: 'var(--lol-bg-elevated)' }}>
      ...
      <div className="flex gap-2">
        {SOLO_DEMO.recentMatches.map((m, i) => (
          <div key={i} className="flex flex-col items-center gap-1">
            <ChampionAvatar champKey={m.key} label={m.label} size={36} ring={m.result === 'V' ? A.gradient : 'rgba(244,63,94,0.8)'} />
            <span className="text-[10px] font-bold" style={{ color: m.result === 'V' ? A.color : '#fb7185' }}>{m.result}</span>
          </div>
        ))}
      </div>
    </motion.div>
